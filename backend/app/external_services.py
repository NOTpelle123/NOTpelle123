...
class JsonTTLCache:
    ...
    def get(self, section: str, key: str) -> Optional[Dict[str, Any]]:
        with self._lock:
            ...
            if self._contains_expired_preview(value):
                return None
            return value
    ...
    def set(self, section: str, key: str, value: Dict[str, Any], ttl_seconds: int) -> None:
        with self._lock:
            ...
            safe_ttl = self._ttl_limited_by_preview(value, ttl_seconds)
            bucket[key] = {
    ...
    def _contains_expired_preview(self, value: Any) -> bool:
        return any(
            expiry <= time.time() + 60
            for expiry in self._preview_expiries(value)
        )

    def _ttl_limited_by_preview(self, value: Any, ttl_seconds: int) -> int:
        expiries = self._preview_expiries(value)
        if not expiries:
            return ttl_seconds
        preview_ttl = int(min(expiries) - time.time() - 60)
        return max(60, min(ttl_seconds, preview_ttl))

    def _preview_expiries(self, value: Any) -> list[int]:
        expiries: list[int] = []

        def collect(item: Any) -> None:
            if isinstance(item, dict):
                preview = str(item.get("preview") or "")
                match = re.search(r"exp=(\d+)", preview)
                if match:
                    try:
