...
class RecommendationEngine:
    ...
    def property_search(
        ...
        """Generate the Discovery results for genre/sliders.

        This is the high-level route used by the Discovery mixer:
        1. Normalize slider targets and selected genres.
        2. Handle the special "new_added" mode separately.
        3. Build a candidate pool from the dataset and locally added songs.
        4. If sliders are active, use K-means/seed ranking to shrink that pool.
        5. Score the remaining candidates with discovery rules.
        6. Build the final cards that the frontend displays.
        """
        ...
        # "Nya tillagda" does not use K-means. It only returns locally added tracks
        # matching the selected genres.
        if mode == "new_added":
            if not genre_values:
                raise ValueError("Välj minst en genre innan du genererar nya tillagda låtar.")
            custom_tracks = self.sqlite_store.list_custom_tracks()
            results = [
                self._with_song_identity(self._with_track_media(item))
        ...
        # User/library context affects scoring later, for example by penalizing recently
        # shown tracks and boosting genres or clusters related to the user's library.
        state = self.store.snapshot()
        custom_tracks = self.sqlite_store.list_custom_tracks()
        context = self._build_library_context(state, custom_tracks)

        # The request dict is passed through the ranking pipeline. It describes what the
        # user asked for and controls which scoring weights are active.
        ...
        # Candidate pool before K-means: dataset songs plus locally added songs, optionally
        # filtered by genre. This step does not choose the final recommendations.
        candidate_frame = self._build_candidate_frame([], request, context, custom_tracks, mode="search")

        if clean_targets:
            # Slider mode: K-means chooses the closest cluster, then the seed/threshold
            # logic returns a smaller candidate window for the normal scorer.
            ...
            if predicted_cluster is not None:
                # Store the chosen cluster for explanations and for a scoring bonus later.
        ...
        # Final scoring adds app-specific rules: genre match, K-means score, small-artist
        # priority, recent-history penalties, and a small amount of controlled randomness.
        ranked = self._score_candidate_frame(candidate_frame, request, context, mode="search")

        # Final assembly turns rows into UI cards and can still filter out songs, for
        # example if an artist fails the small-artist check from external metadata.
    ...
    def _build_candidate_frame(
        ...
        """Create the raw song pool that later ranking steps can choose from."""
        # Resolve genre labels to dataset genre values. If genres are selected, the
        # dataset pool is filtered before any ranking is done.
        ...
        # Make sure every dataset row has a K-means cluster id available for later steps.
        ...
        # For very broad searches, keep the pool large but bounded so scoring stays fast.
        ...
        # Normalize dataset rows into the common candidate shape used by the scorer.
        ...
        # Add locally submitted tracks to the same candidate pool. They can be filtered
        # by genre too, but often have less complete audio metadata than dataset rows.
        ...
        # Fill required columns so downstream ranking can treat dataset and custom rows
        # the same way without repeated missing-column checks.
        for feature in FEATURE_COLUMNS:
    ...
    def _score_candidate_frame(
        ...
        """Assign the app's final ranking score before card assembly.

        K-means can narrow and score the candidate window before this method, but this
        scorer decides the final order by combining genre fit, slider fit, cluster fit,
        small-artist priority, user history, and light randomness.
        """
        if frame.empty:
            return frame.copy()
        ...
        # Early mainstream filter for dataset rows. Custom rows are kept because users
        # explicitly submitted them.
        ...
        # Recent-history penalties make repeated generations less repetitive.
        ...
        # Genre bonus: selected genres should rise in search results.
        if mode == "search":
        ...
        # Text/intent bonus is mostly disabled for slider-only Discovery, but kept for
        # modes that still use tokens.
        ...
        # K-means cluster bonus: when sliders predicted a target cluster, songs in that
        # cluster get an extra boost.
        ...
        # K-means seed/threshold score from rank_frame_by_selected_features.
        ...
        # Small-artist priority is a major part of the prototype's concept.
        ...
        # Mainstream penalty is separate from the early filter. It can push popular
        # artists down even if they were not removed.
        if not self.small_artist_filter_paused:
        ...
        # Small randomness keeps repeated generations from being identical.
    ...
    def _assemble_results(
        ...
        """Turn ranked rows into final cards shown in the UI.

        A high-ranked row can still disappear here if it is a duplicate, fails the
        small-artist profile check, fails a year filter, or cannot pass fallback rules.
        """
        if ranked_frame.empty:
            return []

        # Delivery can shuffle a top window with weights. That means ranking_score is
        # important, but the final visible order is not always a strict sorted list.
        ...
        for _, row in delivery_frame.head(240).iterrows():
            # Basic uniqueness: one card per track and artist in a result set.
            ...
            # This is where external metadata can remove a song. For example, a perfect
            # slider seed can be skipped if Deezer/Last.fm says the artist is too big.
        ...
        # Fallback: if strict rules produce nothing, relax the final pass slightly.
